...
def cnmcts(path_generator: PathGenerator, level: int = 1, bandwidth: int = 50):
    if level == 0:
        while not path_generator.is_finished():
            if code(path_generator.current_position) in path_generator.states_actions.keys():
                move = random.choice(
                    path_generator.states_actions[code(path_generator.current_position)]
                )
                new_cell = continuous_cell_selector(path_generator.current_position, move)
            else:
        ...
        return path_generator.trajectory, path_generator.actions, path_generator.get_score()

    else:
        while not path_generator.is_finished():
            # scores_list = list()
            if not code(path_generator.current_position) in path_generator.states_actions.keys():
                path_generator.states_actions[code(path_generator.current_position)] = list()
                for _ in range(bandwidth):
                    ...
                    path_generator.states_actions[code(path_generator.current_position)].append(move)
            moves_list = path_generator.states_actions[code(path_generator.current_position)]
            ...
            if path_generator.current_steps >= len(path_generator.best_course_of_actions):
                path_generator.actions = path_generator.best_course_of_actions[:][:-1]
                path_generator.trajectory = path_generator.best_trajectory[:][:-1]
                path_generator.current_steps = len(path_generator.best_course_of_actions) - 1
    ...
    return path_generator.best_trajectory, path_generator.best_course_of_actions, path_generator.best_score

def crbnmcts(path_generator: PathGenerator, level: int = 1, bandwidth: int = 50, move: np.ndarray = None, new_position: tuple = None):
    if level == 0:
        assert not (move is None), "'move' must be specified for cRbNMCTS level 0"
        path_generator.update(move, new_position)
        return path_generator.trajectory, path_generator.actions, np.linalg.norm(np.array(path_generator.goal) - np.array(new_position))
        ...
        while not path_generator.is_finished():
            ...
            for move_index, move in enumerate(moves_list):
                ...
                if path.is_finished() and (path.get_score() < path_generator.best_score):
                    path_generator.best_trajectory = trajectory[:]
                    path_generator.best_course_of_actions = actions_list[:]
                    path_generator.best_score = path.get_score()
            if best_move is None:
                best_move = path_generator.best_course_of_actions[path_generator.current_steps]
                best_new_cell = path_generator.best_trajectory[path_generator.current_steps + 1]
