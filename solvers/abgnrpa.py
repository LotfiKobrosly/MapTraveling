...
def abgnrpa_step(position, goal, policy, heuristic_values, score) -> float:
    ...
    if mixture_data:
        ...
        probabilities = np.array([np.exp(model.predict(np.array([position[0], position[1], angle]).reshape((1, -1)))[0] / TAU + heuristic_values.get(position, goal, 2 * np.pi * angle)) for angle in normalized_angles])
        probabilities /= np.sum(probabilities)
        normalized_angle = np.random.choice(normalized_angles, size=1, p=probabilities, replace=False)[0]
